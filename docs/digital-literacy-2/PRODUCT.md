...
- Focused and review-all assessment modes share actual answers and seven topic counts; a topic link opens its first unanswered question. Drafts retain the current question and review mode. Assessment drafts and results use `sessionStorage` in the learner's tab, with a clear-assessment control. Slide progress uses the shared `VubProgress` localStorage layer. The home continue action resumes the saved week; its progress counts lessons viewed to the end, not demonstrated mastery. Worksheet answers remain in the open page and are included in printing. There is no backend grading, automatic instructor submission, instructor results dashboard or durable learner-record service.
...
Retain VUB Learning's name, existing seal, navy-and-gold identity and established course interaction language. This is an existing-brand extension. The user’s approved preference is a visually rich modern learning app with substantial illustration, strong type hierarchy, coherent course navigation and meaningful progress. Apply that preference consistently within this course. Language is concrete, respectful and task-centred; examples connect to useful everyday technology decisions. Pricing, positioning and learner-facing policy remain outside incidental design edits.
...
`HANDOFF.md` records media provenance: Explain Video Generator was trialled separately; the initial ElevenLabs authorization error was repaired; the earlier delivery used Sarah narration and HyperFrames production. The current redesign uses the user-selected Britt — Mild Appalachian Male Voice (`iKrofGyA12WC0e6AhZ8B`); Sandra names the external audio/video checking toolkit, not the narrator. The trial is not a required learner dependency. This learning-app composition leaves the existing Britt-narrated videos unchanged. `review/` contains captured views and print proofs; verification reports record their own scope. These artifacts do not establish live deployment or certify accessibility by themselves.

`REDESIGN.md` records the user-authorized visual-workshop direction and task-first composition. `review/redesign/` contains captured desktop/mobile examples; these captures alone do not establish that finish review, CI or accessibility checks passed. The user approved a combination of all three modern learning-app comps in `.impeccable/mocks/assessment-{a,b,c}.png`: A supplies navy course navigation and illustrated welcome, B the focused illustrated assessment, and C seven-topic navigation and real progress. Their sidecars record approval, and `.impeccable/mocks/README.md` inventories the fidelity requirements. Current screenshots are in `review/learning-app/`; `.impeccable/asset-manifest.json` records the three generated decorative illustration assets. The local quality gate passed with 60 browser tests; independent finish review returned ship. See VALIDATION.md for evidence and verification boundaries. No testimonials, certification claims or production-service capabilities should be invented.
