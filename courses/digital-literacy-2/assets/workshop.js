...
(() => {
  ...
  document.querySelectorAll('[data-workshop]').forEach(root=>{
    ...
    root.addEventListener('click',event=>{
      ...
      if(a==='privacy'){q('[data-shared-event]').replaceChildren();const title=document.createTextNode(v==='busy'?'Busy':'Library practice');const line=document.createElement('span');line.textContent=v==='busy'?'2:00–3:00':'2:00–3:00 · Room A';q('[data-shared-event]').append(title,line);say(root,v==='busy'?'Your partner sees availability. The title and location remain private in this model.':'Your partner can now see the title and location. Share this only when the task requires it.');}
